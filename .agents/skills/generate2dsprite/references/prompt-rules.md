...
Preserve-scale means: fixed-grid cut, chroma-key cleanup, crop detected subject for placement, do not resize the subject by bbox, paste it back into a same-size source cell on a shared feet/bottom anchor, then resize the whole cell to the output size. QC should check `scale_changed=false`, no `paste_clamped` frames, no `edge_touch_frames`, and stable feet/bottom alignment.
