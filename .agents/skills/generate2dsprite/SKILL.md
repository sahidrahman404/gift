...
- **Generate raw images via the active agent's image route**:
  - **Codex CLI** (preferred when running under Codex): use the built-in `image_gen` tool. Reference images via `view_image` first.
  - **Claude Code / any other CLI without built-in image gen**: shell out to `python .agents/tools/gen-image.py "<prompt>" <output.png>`. The daemon routes to Gemini or OpenAI using your stored API key. Reference images via `--ref <path>` (can be repeated). Magenta-bg auto-injection is ON by default — for sprites that's what you want; for non-sprite backgrounds pass `--no-magenta-bg`.
  - Both paths produce equivalent PNGs and feed into the same post-processing scripts (`generate2dsprite.py process`). Don't mix paths within one project to keep style consistent.
- Do not create raw sprite art with Three.js, Canvas, SVG, HTML/CSS drawing, PIL shape drawing, procedural geometry, placeholder primitives, or code-rendered screenshots. Runtime code may display finished generated assets, and scripts may make layout guides or postprocess generated images, but requested sprite art must originate from the active image route above.
- When the user provides or implies a visual reference: under Codex, use `view_image` then built-in image edit/reference semantics. Under other CLIs, pass `--ref <path>` to `gen-image.py` — the daemon handles base64-inlining for Gemini and the `/images/edits` upload for OpenAI.
