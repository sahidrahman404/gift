...
def split_grid(
    ...
    preserve_source_size: tuple[int, int] | None = None
    for row in range(rows):
        for col in range(cols):
            ...
            if preserve_source_size is None:
                preserve_source_size = frame.size
    ...
    if scale_strategy == "preserve":
        source_width, source_height = preserve_source_size or (cell_width, cell_height)
        bbox_centers = []
        for info in frame_info:
            bbox = info.get("crop_bbox")
            if bbox:
                x0, _y0, x1, _y1 = bbox
                bbox_centers.append((x0 + x1) / 2)
        shared_center_x = (
            sorted(bbox_centers)[len(bbox_centers) // 2]
            if bbox_centers
            else source_width / 2
        )
        if align in {"bottom", "feet"}:
            source_pad = max(0, int(source_height * (1 - fit_scale) * 0.5))
            shared_feet_y = source_height - source_pad
        else:
            shared_feet_y = source_height / 2

        frames: list[Image.Image] = []
        for index, frame in enumerate(cropped_frames):
            frame_width, frame_height = frame.size
            source_canvas = Image.new("RGBA", (source_width, source_height), (0, 0, 0, 0))
            if frame_width > 0 and frame_height > 0:
                paste_x = int(round(shared_center_x - frame_width / 2))
                if align in {"bottom", "feet"}:
                    paste_y = int(round(shared_feet_y - frame_height))
                else:
                    paste_y = int(round(shared_feet_y - frame_height / 2))
                unclamped_paste = [paste_x, paste_y]
                paste_x = max(0, min(source_width - frame_width, paste_x))
                paste_y = max(0, min(source_height - frame_height, paste_y))
                paste_clamped = [paste_x, paste_y] != unclamped_paste
                source_canvas.paste(frame, (paste_x, paste_y), frame)
                aligned_bbox = source_canvas.getbbox()
                source_canvas = source_canvas.resize((cell_size, cell_size), Image.Resampling.LANCZOS)
                frame_info[index]["output_size"] = [cell_size, cell_size]
                frame_info[index]["preserved_subject_size"] = [
                    max(1, int(round(frame_width * cell_size / source_width))) if source_width else 0,
                    max(1, int(round(frame_height * cell_size / source_height))) if source_height else 0,
                ]
                frame_info[index]["paste_position"] = [paste_x, paste_y]
                frame_info[index]["unclamped_paste_position"] = unclamped_paste
                frame_info[index]["paste_clamped"] = paste_clamped
                frame_info[index]["aligned_bbox"] = list(aligned_bbox) if aligned_bbox else None
                frame_info[index]["scale_changed"] = False
                frame_info[index]["edge_touch"] = bbox_touches_edge(
                    aligned_bbox,
                    source_width,
                    source_height,
                    edge_touch_margin,
                ...
                frame_info[index]["preserved_subject_size"] = [0, 0]
                frame_info[index]["paste_position"] = [0, 0]
                frame_info[index]["unclamped_paste_position"] = [0, 0]
                frame_info[index]["paste_clamped"] = False
                frame_info[index]["aligned_bbox"] = None
                frame_info[index]["scale_changed"] = False
            frames.append(source_canvas)

        for info in frame_info:
            info["scale_strategy"] = "preserve"
            info["preserve_source_size"] = [source_width, source_height]
            info["shared_center_x"] = shared_center_x
            info["shared_feet_y"] = shared_feet_y
    ...
    for index, frame in enumerate(cropped_frames):
        ...
        if frame_width > 0 and frame_height > 0:
            ...
            frame_info[index]["output_size"] = [0, 0]
            frame_info[index]["paste_position"] = [0, 0]
        frame_info[index]["scale_strategy"] = "fit"
        frame_info[index]["scale_changed"] = frame_width > 0 and frame_height > 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    process_parser.add_argument(
        ...
        help=(
            ...
            "preserve keeps raw-cell scale and only translates the subject to a shared anchor before resizing the whole cell"
        ),
    )
